...
#
# DESCRIPTION
# APII script used to delete macOS policie(s) by their ID
#	
####################################################################################################
#
...
echo ""
read -p "Policy IDs (comma seperated no spaces): " policyID
...
#
####################################################################################################
curl -ksu "$username":"$password" -H "content-type: text/xml" "$server"/JSSResource/policies/id/{$policyID} -X DELETE